using Test
using LuxorLayout
using LuxorLayout: byte_description, LIMIT_fsize_read_svg # Not public
import Luxor
using Luxor: Drawing, O, snapshot, readsvg, placeimage
using Luxor: blend, addstop, setblend, paint, setmode, currentdrawing
using Luxor: paint_with_alpha

# We can play more with this later if we implement
# cairo_get_inkextent().
...
svgfiles = [nam for nam in readdir(joinpath(@__DIR__, "Santana"), join=true) if endswith(nam, ".svg")]
